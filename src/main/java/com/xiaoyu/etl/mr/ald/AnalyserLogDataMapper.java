...
import com.xiaoyu.etl.util.LoggerUtil;
...
public class AnalyserLogDataMapper extends Mapper<LongWritable, Text, NullWritable, Put> {
		...
		if (StringUtils.isNotBlank(serverTime)) {
			...
			for (Map.Entry<String, String> entry : clientInfo.entrySet()) {
				if (StringUtils.isNotBlank(entry.getKey())
					...
					put.addColumn(family, Bytes.toBytes(entry.getKey()),
	 ...
	 * @param serverTime
	 * @return
	 * //crc32转换rowkey减少rowkey长度处理（可以采用其它自定义算法）
	 */
